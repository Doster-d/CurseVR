...
// "Updated to new Input System" || version 1.0.2
...
public class FirstPersonController : MonoBehaviour
{
    ...
    [Header("Zoom is controlled by the Zoom action in your Input Actions")]
    ...
    [Header("Sprint is controlled by the Sprint action in your Input Actions")]
    ...
    [Header("Jump is controlled by the Jump action in your Input Actions")]
    ...
    [Header("Crouch is controlled by the Crouch action in your Input Actions")]
    ...
    // Add this field at the top of the class with other private variables
    ...
    private void Awake()
    {
        ...
        originalWalkSpeed = walkSpeed; // Store the original walk speed

        // Set up Input Actions
        if (inputActions == null)
        {
            Debug.LogError("Input Actions asset is not assigned to the FirstPersonController!");
            return;
        }
    ...
    private void Update()
    {
        ...
        if (enableCrouch)
        {
            if (!holdToCrouch)
            {
                // Toggle crouch is handled by the OnCrouch callback
            }
            else
            {
                // Hold to crouch
                bool wasPressed = crouchInput;
                crouchInput = crouchAction.ReadValue<float>() > 0.5f;
                
                // Only toggle crouch state on changes
                if (wasPressed != crouchInput)
                {
        ...
        // Handle zoom input
        zoomInput = zoomAction.ReadValue<float>() > 0.5f;
        ...
        if (enableZoom)
        {
            // Handle zoom based on input and settings
            if (!holdToZoom && !isSprinting)
            {
                // Toggle zoom handled by callback
            }
    ...
    {
        if (enableCrouch && !holdToCrouch)
        {
            // Toggle crouch state
            isCrouched = !isCrouched;
            Debug.Log($"Crouch toggled. isCrouched: {isCrouched}"); // Debug log
            Crouch();
        }
    }
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            if (enableSprint && sprintInput && sprintRemaining > 0f && !isSprintCooldown && !isCrouched) // Add check for !isCrouched
            {
            ...
            // Apply extra gravity to keep the player grounded
            if (isGrounded)
            {
    ...
    private void Crouch()
    {
        Debug.Log($"Crouch method called. Current state - isCrouched: {isCrouched}, current scale Y: {transform.localScale.y}"); // Debug log
        ...
        if(isCrouched)
        {
            ...
            Debug.Log($"Crouching - New scale Y: {transform.localScale.y}, New speed: {walkSpeed}"); // Debug log
        }
        else
        {
            // Stand up
            ...
            Debug.Log($"Standing - New scale Y: {transform.localScale.y}, New speed: {walkSpeed}"); // Debug log
        }
    }
...
// Custom Editor
    ...
    public override void OnInspectorGUI()
    {
        ...
        // Input Actions field
        EditorGUILayout.PropertyField(SerFPC.FindProperty("inputActions"));
        EditorGUILayout.HelpBox("This controller uses the new Input System. Please assign your Input Actions asset here.", MessageType.Info);
        EditorGUILayout.Space();
        ...
        EditorGUILayout.HelpBox("Zoom is controlled by the Zoom action in your Input Actions asset", MessageType.Info);
        ...
        EditorGUILayout.HelpBox("Sprint is controlled by the Sprint action in your Input Actions asset", MessageType.Info);
        ...
        EditorGUILayout.HelpBox("Jump is controlled by the Jump action in your Input Actions asset", MessageType.Info);
        ...
        EditorGUILayout.HelpBox("Crouch is controlled by the Crouch action in your Input Actions asset", MessageType.Info);
